...
# Check if wp-config.php exists, create it if not
if [ ! -f /var/www/html/wp-config.php ]; then
    if [ -f /var/www/html/wp-config-sample.php ]; then
        echo "Creating wp-config.php from wp-config-sample.php..." | tee -a $LOG_FILE
        sudo mv /var/www/html/wp-config-sample.php /var/www/html/wp-config.php
        sudo chmod 640 /var/www/html/wp-config.php
        sudo chown www-data:www-data /var/www/html/wp-config.php
        check_exit_status "create wp-config.php"
    else
