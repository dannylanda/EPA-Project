...
systemctl status mariadb >> /root/EPA/testing.txt # Appends MariaDB status to the log file
...
sudo php -v >> /root/EPA/testing.txt # Logs the installed PHP version
...
sudo bash /root/EPA/ssl-certbot-setup.sh
