...
sudo systemctl status nginx > /root/EPA-Project/testing.txt # Logs the status of Nginx
...
systemctl status mariadb >> /root/EPA-Project/testing.txt # Appends MariaDB status to the log file
...
sudo php -v >> /root/EPA-Project/testing.txt # Logs the installed PHP version
...
sudo bash /root/EPA-Project/ssl-certbot-setup.sh
